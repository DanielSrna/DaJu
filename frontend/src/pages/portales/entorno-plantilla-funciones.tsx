...
/** Funciones adicionales: catálogo predefinido + pedidos; admin responde. */
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      <Button
        variant="ghost"
        size="sm"
        onClick={() => (window.location.href = `/cliente/plantillas/${id}`)}
      >
        <ArrowLeft className="size-4" /> Volver al resumen
      </Button>
      <h1 className="mt-3 text-2xl font-bold">Funciones adicionales</h1>
      <p className="mt-1 text-sm text-muted-foreground">
        Suma funciones de la lista predefinida o pide una a tu medida. El
        equipo confirma el alcance y el costo antes de que pagues.
      </p>
      ...
      {id && (
        <div className="mt-6">
          <CatalogoFunciones familia="espacio" id={id} onSolicitada={cargar} />
        </div>
      )}
